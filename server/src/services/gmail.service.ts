...
export async function sendGmailMessage(input: {
  ...
  let accessToken =
    (await getGmailAccessToken(input.from)) || input.accessToken;

  if (!accessToken) {
    throw new Error(
      "Gmail access token required from /send body (accessToken) or userintegrations"
    );
  }
  ...
  if (
    ...
    accessToken = await getGmailAccessToken(input.from, true);
    if (accessToken) {
      ...
      if (!retry.ok) {
        throw new Error(retryData?.error?.message || "Failed to send Gmail message");
      }
      console.log("mail send");
      return { messageId: retryData.id, threadId: retryData.threadId };
    }
  }
